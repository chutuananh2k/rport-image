...
#
#          BUGS: https://github.com/realvnc-labs/rportd-installer/issues
#
#     COPYRIGHT: (c) 2023 by the RealVNC Team,
#
#       LICENSE: MIT
...
on_fail() {
  ...
  echo "https://github.com/realvnc-labs/rportd-installer/discussions/categories/help-needed"
  echo ""
}
debug() {
...
install_guacd() {
  ...
  GUACD_PKG=rport-guacamole_1.5.0_${ID}_${VERSION_CODENAME}_$(uname -m).deb
  GUACD_DOWNLOAD=https://github.com/realvnc-labs/rport-guacamole/releases/download/1.5.0/${GUACD_PKG}
...
#----------------------------------------------------------------------------------------------------------------------
install_novnc() {
  if [ -d /var/lib/rport/noVNC-1.4.0 ]; then
    throw_info "NoVNC already installed"
    return 0
  fi
  if [ -n "$NOVNC_ROOT" ]; then
    NOVNC_DOWNLOAD='https://github.com/novnc/noVNC/archive/refs/tags/v1.4.0.zip'
...
activate_novnc() {
  ...
  NOVNC_ROOT='/var/lib/rport/noVNC-1.4.0'
...
if [ -z "$USE_VERSION" ];then
  ...
  DOWNLOAD_URL="https://github.com/realvnc-labs/rport/releases/download/${USE_VERSION}/rportd_${USE_VERSION}_Linux_$(uname -m).tar.gz"
...
if [ -z "$FQDN" ]; then
  ...
  DNS_CREATED=1
  PUBLIC_FQDN=1
  throw_info "Creating random FQDN ${FQDN} on Freedns *.users.rport.io."
  wait_for_dns_ready
...
if [ "$API_PORT" -ne 443 ]; then
    ...
    throw_info "No certificates created. You can very likely use the built-in ACME."
fi
# If none of the above will create a certificate, and if HTTP port is 443, the built-in ACME will be activated.
# The create-config routines will check the existence of the SSL_CERT_FILE variable.
...
sed -i "s|#base_url = .*|base_url = \"https://${FQDN}:${API_PORT}\"|g" $CONFIG_FILE
...
elif [ "$API_PORT" -eq 443 ];then
    # Enable the built-in ACME
    sed -i "s|#enable_acme =.*|enable_acme = true|g" $CONFIG_FILE
    sed -i "s|#tunnel_enable_acme =.*|tunnel_enable_acme = true|g" $CONFIG_FILE
    sed -i "s|#tunnel_host = .*|tunnel_host = \"${FQDN}\"|g" $CONFIG_FILE
fi
...
enable_email_2fa() {
  sed -i "s|#two_fa_token_delivery.*|two_fa_token_delivery = \"https://free-2fa-sender.rport.io\"|g" /etc/rport/rportd.conf
